...
C = S["cards"]
blocks = [
 ("hook", "f_open", alen("f_open") + 0.9, None, 0.45),
 ("card", card("t1", C["t1"]["big"], C["t1"]["small"]), 2.8, None, 0),
 ("card", card("q1", C["q1"]), 1.3, None, 0),
 ("voice", "f_s1", alen("f_s1") + 0.5, D / "../ep46-renvyle/s3-corridor.jpg", 0.15),
 ("card", card("q2", C["q2"]), 1.3, None, 0),
 ("voice", "f_s2", alen("f_s2") + 0.5, None, 0.15),
 ("card", card("q3", C["q3"]), 1.3, None, 0),
 ("voice", "f_s3", alen("f_s3") + 0.6, D / "../ep46-renvyle/s1-door.jpg", 0.15),
 ("card", card("q4", C["q4"]), 1.3, None, 0),
 ("voice", "f_s4", alen("f_s4") + 0.5, None, 0.15),
 ("card", card("t4", C["t4"]["big"]), 2.0, None, 0),
 ("voice", "f_s5", alen("f_s5") + 0.8, None, 0.15),
...
for i, (w, s, e) in enumerate(w3):
    if w.lower().startswith("shut"):
        sting_t = marks["f_s2"] + s - 0.3; break
plan = {"D": round(D_total, 3), "marks": {k: round(v, 3) for k, v in marks.items()},
        "sting_ms": int(sting_t * 1000),
        "hiss_end": round(marks["f_s5"] + alen("f_s5") + 0.4, 3)}
