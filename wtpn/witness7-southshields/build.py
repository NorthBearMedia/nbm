...
C = S["cards"]
blocks = [
 ("hook", "s_open", alen("s_open") + 0.9, None, 0.45),
 ("card", card("t1", C["t1"]["big"], C["t1"]["small"]), 3.2, None, 0),
 ("card", card("q1", C["q1"]), 1.5, None, 0),
 ("voice", "s_s1", alen("s_s1") + 0.5, D / "../ep45-sauchie/s6-house-night.jpg", 0.15),
 ("card", card("q2", C["q2"]), 1.5, None, 0),
 ("voice", "s_s2", alen("s_s2") + 0.5, None, 0.15),
 ("card", card("q3", C["q3"]), 1.5, None, 0),
 ("voice", "s_s3", alen("s_s3") + 0.6, D / "../ep45-sauchie/s3-parlour.jpg", 0.15),
 ("card", card("q4", C["q4"]), 1.5, None, 0),
 ("voice", "s_s4", alen("s_s4") + 0.5, None, 0.15),
 ("card", card("t4", C["t4"]["big"]), 2.4, None, 0),
 ("voice", "s_s5", alen("s_s5") + 0.8, None, 0.15),
 ("card", card("t5", C["t5"]["big"]), 2.4, None, 0),
 ("card", card("t6", C["t6"]["big"]), 2.2, None, 0),
]
...
for i, (w, s, e) in enumerate(w3):
    if w.lower().startswith("carving"):
        sting_t = marks["s_s3"] + s - 0.3; break
plan = {"D": round(D_total, 3), "marks": {k: round(v, 3) for k, v in marks.items()},
        "sting_ms": int(sting_t * 1000),
        "hiss_end": round(marks["s_s5"] + alen("s_s5") + 0.4, 3)}
