#!/usr/bin/env python3
"""WITNESS 19 — Hexham Heads/Dr Anne Ross. Hook-standard build (bible 28 Aug)."""
...
def card(name, lines, small=None):
    ...
    for ln in lines:
        ...
        if w > W - 80: print(f"  !! CARD OVERFLOW {name}: {ln!r} is {w:.0f}px (max {W-80})")
        d.text(((W - w) / 2, y), ln, font=f, fill=(216, 214, 205)); y += 78
    if small:
        y += 40
        for ln in small:
...
def seg_words(k):
    ...
    return [(w, s, e) for w, s, e in words if any(c.isalnum() for c in w) or w in ("—", "--")]
...
C = S["cards"]
blocks = [
 ("hook", "a_open", alen("a_open") + 0.9, None, 0.45),
 ("card", card("t1", C["t1"]["big"], C["t1"]["small"]), 3.0, None, 0),
 ("card", card("q1", C["q1"]), 1.4, None, 0),
 ("voice", "a_s1", alen("a_s1") + 0.5, D / S["stills"]["a_s1"], 0.15),
 ("card", card("q2", C["q2"]), 1.4, None, 0),
 ("voice", "a_s2", alen("a_s2") + 0.5, D / S["stills"]["a_s2"], 0.15),
 ("card", card("q3", C["q3"]), 1.4, None, 0),
 ("voice", "a_s3", alen("a_s3") + 0.6, None, 0.15),
 ("card", card("q4", C["q4"]), 1.4, None, 0),
 ("voice", "a_s4", alen("a_s4") + 0.5, D / S["stills"]["a_s4"], 0.15),
 ("card", card("t4", C["t4"]["big"]), 3.0, None, 0),
 ("voice", "a_s5", alen("a_s5") + 0.8, None, 0.15),
 ("card", card("t5", C["t5"]["big"]), 2.0, None, 0),
 ("card", card("t6", C["t6"]["big"]), 1.4, None, 0),
]
...
for i, (w, s, e) in enumerate(w3):
    if w.lower().startswith("study"):
        sting_t = marks["a_s4"] + s - 0.3; break
plan = {"D": round(D_total, 3), "marks": {k: round(v, 3) for k, v in marks.items()},
        "sting_ms": int(sting_t * 1000),
        "hiss_end": round(marks["a_s5"] + alen("a_s5") + 0.4, 3)}
