...
C = S["cards"]
blocks = [
 ("hook", "b_open", alen("b_open") + 0.9, None, 0.45),
 ("card", card("t1", C["t1"]["big"], C["t1"]["small"]), 3.2, None, 0),
 ("card", card("q1", C["q1"]), 1.5, None, 0),
 ("voice", "b_s1", alen("b_s1") + 0.5, D / "../ep41-cooneen/s1-bedroom.jpg", 0.15),
 ("card", card("q2", C["q2"]), 1.5, None, 0),
 ("voice", "b_s2", alen("b_s2") + 0.5, None, 0.15),
 ("card", card("q3", C["q3"]), 1.5, None, 0),
 ("voice", "b_s3", alen("b_s3") + 0.6, None, 0.15),
 ("card", card("q4", C["q4"]), 1.5, None, 0),
 ("voice", "b_s4", alen("b_s4") + 0.5, D / "../ep45-sauchie/s3-parlour.jpg", 0.15),
 ("card", card("t4", C["t4"]["big"]), 2.4, None, 0),
 ("voice", "b_s5", alen("b_s5") + 0.8, D / "../ep45-sauchie/s6-house-night.jpg", 0.15),
 ("card", card("t5", C["t5"]["big"]), 1.8, None, 0),
 ("card", card("t6", C["t6"]["big"]), 2.6, None, 0),
]
...
for i, (w, s, e) in enumerate(w3):
    if w.lower().startswith("up"):
        sting_t = marks["b_s3"] + s - 0.3; break
plan = {"D": round(D_total, 3), "marks": {k: round(v, 3) for k, v in marks.items()},
        "sting_ms": int(sting_t * 1000),
        "hiss_end": round(marks["b_s5"] + alen("b_s5") + 0.4, 3)}
