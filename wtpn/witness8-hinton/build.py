...
C = S["cards"]
blocks = [
 ("hook", "h_open", alen("h_open") + 0.9, None, 0.45),
 ("card", card("t1", C["t1"]["big"], C["t1"]["small"]), 2.8, None, 0),
 ("card", card("q1", C["q1"]), 1.3, None, 0),
 ("voice", "h_s1", alen("h_s1") + 0.5, D / "../ep42-borley/fr_s2-rectory_5.jpg", 0.15),
 ("card", card("q2", C["q2"]), 1.3, None, 0),
 ("voice", "h_s2", alen("h_s2") + 0.5, None, 0.15),
 ("card", card("q3", C["q3"]), 1.3, None, 0),
 ("voice", "h_s3", alen("h_s3") + 0.6, D / "../ep41-cooneen/s1-bedroom.jpg", 0.15),
 ("card", card("q4", C["q4"]), 1.3, None, 0),
 ("voice", "h_s4", alen("h_s4") + 0.5, D / "../ep46-renvyle/s1-door.jpg", 0.15),
 ("card", card("t4", C["t4"]["big"]), 2.0, None, 0),
 ("voice", "h_s5", alen("h_s5") + 0.8, None, 0.15),
...
for i, (w, s, e) in enumerate(w3):
    if w.lower().startswith("slapped"):
        sting_t = marks["h_s3"] + s - 0.3; break
plan = {"D": round(D_total, 3), "marks": {k: round(v, 3) for k, v in marks.items()},
        "sting_ms": int(sting_t * 1000),
        "hiss_end": round(marks["h_s5"] + alen("h_s5") + 0.4, 3)}
